...
void idct_test(const char *test_name, short in[], short want[]) {
  ...
  const int vectorSize = IDCT_blockDim * IDCT_blockDim;
  const int streamSize = 1;
  size_t sizeBytes = vectorSize * streamSize * sizeof(short);
  short *outVector = malloc(sizeBytes);

  IDCT(streamSize, in, sizeBytes, outVector, sizeBytes);

  print_array("got = ", outVector, SIZE);

  if (compare_arrays(outVector, want, DATA_DIM)) {
    printf("%s is PASSED\n", test_name);
  } else {
    printf("%s is FAILED\n", test_name);
  }
