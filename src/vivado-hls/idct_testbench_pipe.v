...
module checker (clock, obl_tdata, obh_tdata, obl_tvalid, obh_tvalid, finish);
reg [`WOUT-1:0] out_reg [63:0];
input wire clock;
input wire [63:0] obl_tdata;
input wire [63:0] obh_tdata;
input wire obl_tvalid;
input wire obh_tvalid;
integer i;
integer fail;
integer ntest;
output reg finish;
...
always @(posedge clock) begin
  ...
  if (obl_tvalid & obh_tvalid) begin
...
checker checker(clock, obl_tdata, obh_tdata, obl_tvalid, obh_tvalid, finish);
...
Top_Fast_IDCT idct(
  ...
  .obl_TREADY(1'b1/*obl_tready*/),
  .obh_TDATA(obh_tdata),
  .obh_TVALID(obh_tvalid),
  .obh_TREADY(1'b1/*obh_tready*/));
