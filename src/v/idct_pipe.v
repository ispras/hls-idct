...
`define TEST(number) \
  ...
  clk <= 1; \
  #1; \
  if (out != `REF``number) begin \
    $display("[FAIL] test #number:"); \
    $display("expected value is 0x%x\nreceived value is 0x%x", `REF``number, out); \
    $finish; \
  end else begin \
    $display("[OK] test #number: out_reg is 0x%x", out); \
  end
...
module Fast_IDCT(input [`WIN*8*8-1:0] in, output [`WOUT*8*8-1:0] out, input clk, input rst);
...
Fast_IDCT idct(`ARRAY_TO_BITVECTOR(b), out, clk, rst);
